...
//! Gradle build module — build.gradle analysis and build/test execution.
...
/// Static Gradle build module.
pub struct GradleBuildModule;

impl GradleBuildModule {
    pub fn new() -> Self {
        Self
    }

    fn analyze(&self, path: &Path) -> Result<BuildMetadata, String> {
        let mut metadata = parse_key_value(path, "build.gradle", "")?;
        metadata.build_system = "Gradle".to_string();
        metadata.project_type = "Gradle_project".to_string();
        Ok(metadata)
    }

    async fn build(&self, path: &Path, _opts: &BuildOptions) -> Result<BuildOutput, String> {
        run_cmd(path, "gradle", &["build"]).await
    }
...
#[async_trait]
impl Actor for GradleBuildModule {
...
"#.replace("__P__", &project_name);
            ...
            BuildModuleMessage::CallTool { reply_to, .. } => {
                let _ = reply_to
                    .send(serde_json::json!({ "error": "gradle module CallTool not yet wired" }));
            }
        }
    }
}
