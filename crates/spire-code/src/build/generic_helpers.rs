...
pub fn hal_meson_var_section(platform: &str, interface_stems: &[String]) -> String {
    let mut entries: Vec<String> = interface_stems
        .iter()
        .map(|stem| format!("        'implementations/{platform}/{stem}_stub.cpp',"))
        .collect();
    // The aggregate HAL the generic pipeline injects — exactly one per platform.
    entries.push(format!(
        "        'implementations/{platform}/ai_trap_hal_{platform}.cpp',"
    ));
    format!(
...
/// Camel-case a platform id for a class-name suffix: `a7s` → `A7s`,
/// `rpi5` → `Rpi5`, `rock3c` → `Rock3c`.
pub fn platform_class_suffix(platform: &str) -> String {
...
/// Render the concrete aggregate-HAL **header** for a newly scaffolded platform:
/// `class AiTrapHal<Plat> : public AiTrapHal` overriding every accessor of the
/// project's `hal/api/ai_trap_hal.hpp` contract. `accessors` is the parsed
/// `(return_type, method_name)` list from that contract.
pub fn generate_aggregate_hal_header(
    platform: &str,
    class_name: &str,
    ...
    h.push_str(&format!(
        "// {sentinel}: ai_trap_hal_{platform}.hpp — {platform} aggregate HAL pending.\n\
         // Owns the {platform} HAL components; the pipeline injects this ONE object.\n\
         #pragma message(\"SPIRE HAL stub needs implementation: ai_trap_hal_{platform}.hpp ({platform})\")\n\n",
        sentinel = SPIRE_HAL_STUB_SENTINEL
    ));
    h.push_str("#pragma once\n\n");
    h.push_str("#include \"hal/api/ai_trap_hal.hpp\"\n\n");
    ...
    for (ret, name) in accessors {
        h.push_str(&format!("    {ret} {name}() override;\n"));
    }
    h.push_str("\nprivate:\n");
    h.push_str("    // TODO: own the platform components (one per accessor above).\n");
    for (ret, name) in accessors {
        // `ICameraHAL&` → member `std::unique_ptr<ICameraHAL> camera_;`
        let ty = ret.trim().trim_end_matches('&').trim();
        h.push_str(&format!("    std::unique_ptr<{ty}> {name}_;\n"));
    }
    h.push_str("};\n\n} // namespace hal\n");
    h
}

/// Render the concrete aggregate-HAL **source** skeleton: construct each
/// component in the ctor and return it by reference from the accessor.
pub fn generate_aggregate_hal_source(
    platform: &str,
    class_name: &str,
    accessors: &[(String, String)],
) -> String {
    let mut s = String::new();
    s.push_str(&format!("// {SPIRE_HAL_STUB_SENTINEL}: ai_trap_hal_{platform}.cpp — {platform} aggregate HAL pending.\n"));
    s.push_str(&format!("#include \"ai_trap_hal_{platform}.hpp\"\n\n"));
    s.push_str("#include <iostream>\n\n");
    s.push_str("namespace hal {\n\n");
    s.push_str(&format!(
        "{class_name}::{class_name}() {{\n    // TODO: construct the {platform} components, e.g.\n"
    ));
    for (ret, name) in accessors {
        let ty = ret.trim().trim_end_matches('&').trim();
        s.push_str(&format!(
            "    //   {name}_ = std::make_unique<{ty}{suffix}>();\n",
            suffix = platform_class_suffix(platform)
        ));
    }
    s.push_str("    std::cerr << \"[");
    s.push_str(class_name);
    s.push_str("] not implemented for ");
    s.push_str(platform);
    s.push_str("\\n\";\n}\n\n");
    s.push_str(&format!("{class_name}::~{class_name}() = default;\n\n"));
    for (ret, name) in accessors {
        s.push_str(&format!(
            "{ret} {class_name}::{name}() {{\n    return *{name}_;\n}}\n\n"
        ));
...
/// Render `app/platform_hal_<plat>.cpp` — the ONE platform-specific binding:
/// construct the platform's aggregate HAL.
pub fn generate_platform_hal_source(platform: &str, class_name: &str) -> String {
    format!(
        "// Generated by hal_add_platform — the ONLY platform-specific binding in\n\
         // the program. The generic app/main.cpp and the pipeline are shared.\n\
         #include \"platform_hal.hpp\"\n\n\
         #include \"hal/implementations/{platform}/ai_trap_hal_{platform}.hpp\"\n\n\
         namespace hal {{\n\n\
         std::unique_ptr<AiTrapHal> create_platform_hal(const PipelineConfig& /*cfg*/) {{\n\
