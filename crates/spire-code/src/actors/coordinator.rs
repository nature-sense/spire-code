                ...
                match rx.await {
                    Ok(Ok(nodes)) => {
                        ...
                        let platforms: Vec<crate::platform::Platform> = nodes
                            ...
                            .collect();
                        // The graph is seeded from the **union** of the stores, so it already
                        // holds the boards as well as the chips. Nothing is appended here:
                        // now that the seed carries the boards, appending would list each
                        // one twice.
                        if !platforms.is_empty() {
                            ...
                            // stores are the source of truth for the toolchain —
                            // fall back to reading them directly so the viewer
                            // always shows every registered entry.
                            platforms_listing(
                                crate::platform::Platform::load_registry().unwrap_or_default(),
                            )
                        }
                    }
                    Ok(Err(e)) => serde_json::json!({"error": e.to_string()}),
