...
pub async fn build_default_registry(
    ...
    register_msg_backend(
        ...
        |tool_name, args, reply| FilesystemMessage::CallTool {
            tool_name,
            args,
            reply_to: reply,
        },
    ...
    register_msg_backend(
        ...
        |tool_name, args, reply| GitMessage::CallTool {
            tool_name,
            args,
            reply_to: reply,
        },
    ...
    register_msg_backend(
        ...
        |tool_name, args, reply| ProcessMessage::CallTool {
            tool_name,
            args,
            reply_to: reply,
        },
    ...
    register_msg_backend(
        ...
        |tool_name, args, reply| SearchMessage::CallTool {
            tool_name,
            args,
            reply_to: reply,
        },
    ...
    register_msg_backend(
        ...
        |tool_name, args, reply| TerminalMessage::CallTool {
            tool_name,
            args,
            reply_to: reply,
        },
    ...
    register_msg_backend(
        ...
        |tool_name, args, reply| BuildManagerMessage::CallTool {
            tool_name,
            args,
            reply_to: reply,
        },
...
fn actor_tool_handler<M, C>(tx: mpsc::Sender<M>, name: String, ctor: C) -> ToolHandler
where
    M: Send + 'static,
...
fn actor_tool_handler_result<M, C>(tx: mpsc::Sender<M>, name: String, ctor: C) -> ToolHandler
where
    M: Send + 'static,
    C: Fn(String, Value, oneshot::Sender<Result<Value, String>>) -> M
        + Send
        + Sync
        + Clone
        + 'static,
    ...
    Arc::new(move |args| {
        ...
        Box::pin(async move {
            ...
            tx.send(msg)
                ...
                .map_err(|e| format!("backend send error: {e}"))?;
            rx.await
                .map_err(|e| format!("backend response error: {e}"))?
        })
    })
}
